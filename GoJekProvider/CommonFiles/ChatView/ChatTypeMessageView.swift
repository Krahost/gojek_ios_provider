...
class ChatTypeMessageView: UIView {
    ...
    func setUpViews() {
        ...
        messageTextView.font = .setCustomFont(name: .light, size: .x16)
    ...
    func setUpConstraint() {
        ...
        //sendBtn
        sendBtn?.trailingAnchor.constraint(equalTo: (sendBtn?.superview?.trailingAnchor)!, constant: -ScreenConstants.proportionalValueForValue(value: 16.0)).isActive = true
        sendBtn?.widthAnchor.constraint(equalToConstant: ScreenConstants.proportionalValueForValue(value: 40.0)).isActive = true
        sendBtn?.heightAnchor.constraint(equalToConstant: ScreenConstants.proportionalValueForValue(value: 40.0)).isActive = true
        sendBtn?.centerYAnchor.constraint(equalTo: (sendBtn?.superview?.centerYAnchor)!, constant: 0.0).isActive = true
    }
}
