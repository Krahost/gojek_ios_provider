...
class PickerManager {
    ...
    func showDatePicker(selectedDate: String?, minDate: Date? = nil ,completionHandler : @escaping (PickerCompletion) -> Void) {
        ...
        CustomPicker.showDatePicker(type: .date, sourceVC: UIApplication.topViewController()!, minDate: minDate, selectedDate: selectedDate) { (selectdValue) in
    ...
    func showTimePicker(selectedDate: String?, minDate: Date? = nil, completionHandler : @escaping (PickerCompletion) -> Void) {
        ...
        CustomPicker.showDatePicker(type: .time, sourceVC: UIApplication.topViewController()!, minDate: minDate, selectedDate: selectedDate) { (selectdValue) in
            completionHandler(selectdValue)
        }
    }
...
fileprivate class CustomPicker: NSObject {
    ...
    static var dummyBackgroundView: UIView = {
        ...
        sView.backgroundColor = .boxColor
        sView.alpha = 1
        return sView
    }()
    
    static let barAccessory = UIToolbar()
        ...
        if let sub = currentVC?.view.subviews {
            for sview in sub {
                if sview.tag != 100 {
                    ...
                    picker.backgroundColor = .boxColor
    ...
    static func showDatePicker(type: UIDatePicker.Mode, sourceVC: UIViewController, minDate: Date?, selectedDate: String?, closure:@escaping (PickerCompletion) -> Void) {
        ...
        datepicker.backgroundColor = .boxColor
        ...
        if let sub = currentVC?.view.subviews {
            for sview in sub {
                if sview.tag != 101 {
                    datepicker.backgroundColor = .boxColor
                    datepicker.alpha = 1
                    currentVC?.view.addSubview(datepicker)
                }
            }
        }
...
//        currentVC?.view.addSubview(datepicker)
        setPickerFrame(pickerView: datepicker, vc: currentVC!)
        sourceVC.present(currentVC!, animated: true, completion: nil)
        completionHandler = { value in
            closure(value)
        }
...
//                handler(dateString)
    ...
    static func setPickerFrame(pickerView: UIView, vc: UIViewController) {
        ...
        dummyBackgroundView.topAnchor.constraint(equalTo: pickerView.topAnchor).isActive = true
    ...
    @objc static func doneClick() {
        UIView.animate(withDuration: 0.5, delay: 0, options: UIView.AnimationOptions.curveEaseIn, animations: {
            ...
            dummyBackgroundView.removeFromSuperview()
