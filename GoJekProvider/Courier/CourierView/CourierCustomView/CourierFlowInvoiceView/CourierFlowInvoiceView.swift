...
class CourierFlowInvoiceView: UIView {
...
//        statusTblVw.register(nibName: CourierConstant.CourierStatusTableViewCell)
    ...
    private func setLocalization(){
        //New
        doneButton.setTitle(CourierConstant.confirmPayment.localized, for: .normal)
        staticTotalPriceLabel.text = CourierConstant.total.localized
        ...
        staticHeightLabel.text = OrdersConstant.height.localized
        staticLengthLabel.text = OrdersConstant.length.localized
        staticWeightLabel.text = OrdersConstant.weight.localized
        staticBreadthLabel.text = OrdersConstant.breadth.localized
        staticDeliveryLabel.text = OrdersConstant.packageType.localized
...
//        staticDeliveryLabel.text = OrdersConstant.deliveryType.localized
//        staticRecieverNameLabel.text = OrdersConstant.receiverName.localized
//        staticReceiverMobileLabel.text = OrdersConstant.receiverMobile.localized
    }
...
//        }
    ...
    @objc func makeCallAction(sender : UIButton){
        AppUtils.shared.call(to: self.requestData?.request?.delivery?.mobile ?? "")
    }
    ...
    @objc func showfareDetails(sender : UIButton){
        ...
        if let data =  self.requestData{
            self.receiptView?.setCourierFlowValues(values: data, index: sender.tag, isOverAll: false)
        }
    ...
    func setValues(values:CourierCheckRequestData, data : DeliveryEntity, isSenderPay : Bool) {
        //New
        pickupLoaction.text = values.request?.sAddress ?? ""
        bookingIDLabel.text = values.request?.bookingId ?? ""
        self.totalPriceLabel.text = "\(values.request?.currency ?? "") \(String(describing: (values.request?.payment?.payable ?? 0).roundOff(2)))"
//        totalPriceLabel.text = data.payment?.total?.rounded().setCurrency()
        ...
        if values.request?.payment_by?.uppercased() == "SENDER" {
    ...
    @IBAction func confirmPayment(sender:UIButton){
        if paymentType == .CASH {
            if isPaid == 0 {
                ...
                ToastManager.show(title: Constant.confirmPayment.localized, state: .error)
            }
        }else{
            onClickConfirm?()
        }
...
extension CourierFlowInvoiceView: UITableViewDataSource {
...
//        return (self.requestData?.data?.first?.deliveries?.count ?? 0)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            let data = self.requestData?.request?.delivery
            ...
            cell.lengthLabel.text = "\(data?.length ?? 0) \(OrdersConstant.cm.localized)"
            cell.lengthView.isHidden = ((data?.length ?? 0) == 0)
            cell.breadthLabel.text = "\(data?.breadth ?? 0) \(OrdersConstant.cm.localized)"
            cell.breadthView.isHidden = ((data?.breadth ?? 0) == 0)
            cell.weightLabel.text = "\(data?.weight ?? 0) \(OrdersConstant.kg.localized)"
            cell.weightView.isHidden = ((data?.weight ?? 0) == 0)
            cell.heightLabel.text = "\(data?.height ?? 0) \(OrdersConstant.cm.localized)"
...
//            }
            ...
            if indexPath.row == current {
                cell.outterView.layer.borderColor = UIColor.courierColor.cgColor
                cell.outterView.layer.borderWidth = 0.5
                ...
                cell.outterView.layer.borderColor = UIColor.lightGray.cgColor
...
extension CourierFlowInvoiceView: UITableViewDelegate {
